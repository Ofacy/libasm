...
ssize_t	pf_print_string(t_format *format, char *string)
{
	...
	libft_strlcat(print, string, i + format->arglen + 1);
	if (format->minus)
	{
		i = format->arglen;
...
{
	if (format->period)
		format->arglen = libft_strnlen(string, format->precision);
	else
		format->arglen = libft_strlen(string);
...
ssize_t	pf_print_null_string(t_format *format)
{
	...
	libft_strlcat(print, "(null)", i + format->arglen + 1);
	if (format->minus)
	{
		i = format->arglen;
		libft_memset(print + i, ' ', format->padlen);
